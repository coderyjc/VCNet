...
from model import ft_net, ft_net_ours, ft_net_dense, ft_net_EF4, ft_net_EF5, ft_net_EF6, ft_net_IR, ft_net_NAS, ft_net_SE, ft_net_DSE, PCB, CPB, ft_net_angle, ft_net_arc
...
if opt.h == opt.w:
    transform_train_list = [
        ...
        transforms.Pad(10),
        transforms.RandomCrop((opt.inputsize, opt.inputsize)),
...
def train_model(model, model_view, criterion, optimizer, scheduler, start_epoch=0, num_epochs=25):
    ...
    for epoch in range(num_epochs - start_epoch):
        ...
        for phase in ['train']:
            ...
            # deep copy the model
            if len(opt.gpu_ids)>1 and epoch % 10 == 0:
...
if not opt.resume:
    ...
    if opt.use_dense:
        ...
        model = ft_net_SE(len(class_names), opt.droprate, opt.stride, opt.pool, circle = opt.circle)
    elif opt.use_DSE:
        ...
        model = ft_net_ours(len(class_names), opt.droprate, opt.stride, circle =opt.circle, ibn=opt.ibn)
...
print(model)
model = train_model(model, model_view,  criterion, optimizer_ft, exp_lr_scheduler,
