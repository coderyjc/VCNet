...
def evaluate(score,ql,qc,gl,gc):
    ...
    #print(good_index)
    CMC_tmp = compute_mAP(index, good_index, junk_index, gc)
    return CMC_tmp

def compute_mAP(index, good_index, junk_index, gc):
    ap = 0
    cp = 0
...
def calculate_result(gallery_feature, gallery_label, gallery_cam, query_feature, query_label, query_cam, result_file, pre_compute_score=None):
    ...
    for i in range(len(query_label)):
        ap_tmp, CMC_tmp, inp_tmp, cp_tmp = evaluate(score[i],query_label[i],query_cam[i],gallery_label,gallery_cam)
        if CMC_tmp[0]==-1:
            continue
        # if CMC_tmp[0]==0:
        #     print(i)
    ...
    str_result = 'Rank@1:%f Rank@5:%f Rank@10:%f mCP:%f mAP:%f \n'%(CMC[0], CMC[4], CMC[9], cp/len(query_label), ap/len(query_label))
    print(str_result)
...
if __name__ == '__main__':
    result = scipy.io.loadmat('pytorch_result_resnet_151.mat')
    result_gallery = scipy.io.loadmat('pytorch_result_gallery_151.mat')
